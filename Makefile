...
watch: server bundle
ifeq ($(MM_DEBUG),)
	cd webapp && $(BUN) run build:watch
else
	cd webapp && $(BUN) run debug:watch
endif
...
ifeq ($(HAS_MM_UTILITIES),)
	...
	cd $(MM_UTILITIES_DIR) && bun install && bun run babel && node mmjstool/build/index.js i18n extract-webapp --webapp-dir $(PWD)/webapp
endif
endif
